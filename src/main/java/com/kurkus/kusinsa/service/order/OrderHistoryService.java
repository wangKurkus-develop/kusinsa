...
public class OrderHistoryService {
    ...
    public Page<OrderHistoryResponse> findAllWithPage(Long userId, int page) {
        Page<OrderHistory> result = historyRepository.findAllWithPage(userId, PageRequest.of(page, HISTORY_SIZE));
        return result.map(h -> OrderHistoryResponse.from(h));
    }

    /**
     * 주문 취소는 배송준비중일떄만 가능하다
    ...
    public void cancel(Long userId, Long historyId) {
        OrderHistory history = historyRepository.getById(historyId);
